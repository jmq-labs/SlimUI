      ...
      if(!o){
       if(!$(this).hasClass("_MENU")){
    	$(this).children(".INPUT_MENU").each(function(){ 
			...
			$(this).children(".MENU_LIST").children().each(function(){ if(!$(this).is("span")){ $(this).addClass("_VOID"); }});
...
function SlimAlert(m,fn){
    SlimLockScr(true,function(){
    	...
    	$(".BLOCK_SCREEN").html("<div class='MSG_WINDOW_CONTAINER'><div class='MSG_WINDOW'><p><label>"+m+"</label></p></div></div>");
    	...
    	window.scrollTo(0,0);	
    	$(".MSG_WINDOW").show("fade",120,function(){ $("#MSG_CONFIRM_BT").focus(); });
	});
}

function SlimLockScr(o,fn){
	if(o){	    
	   var BKS = "<div class='BLOCK_SCREEN'></div>"; 
	   if($(".BLOCK_SCREEN").length == 0){ 
	   		$('body').append(BKS);		
			$(".BLOCK_SCREEN").height($(document).height());
	   		$(".BLOCK_SCREEN").show("fade",100,function(){ $(".MAIN").contents().find("body").addClass("BLUR"); if(fn) fn(); });
	   }	   
	}else{
	   $(".BLOCK_SCREEN").hide("fade",100,function(){ 
	   		$(".MAIN").contents().find("body").removeClass("BLUR");
			$(".BLOCK_SCREEN").remove();
			if(fn) fn();
		});
	}
